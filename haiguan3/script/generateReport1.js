const report = async () => {
  //登录
  let iframe = document.getElementById("layui-layer-iframe3");
  iframe.contentWindow.document.getElementsByTagName("label")[2].click();
  await $timeout(1);
  document.getElementsByClassName("layui-layer-btn0")[1].click();
  await $timeout(1);
  document.getElementsByClassName("layui-layer-btn1")[0].click();
  //点击菜单
  document.getElementById("deskmenu").children[1].children[0].click();
  document.getElementById("P_HCDECL_MGR").children[0].click();
  ...
  // 输入查询时间
  let tmpNode = await $find_elements({ by: "id", value: "_easyui_textbox_input5", script: "$dom.readonly = false" });
  tmpNode = await $find_elements({ by: "id", value: "_easyui_textbox_input7", script: "$dom.readonly = false" });
  tmpNode = await $find_elements({
    by: "id",
    value: "_easyui_textbox_input5",
    script: `$dom.value = "${dayjs(searchTime[0]).format("YYYY-MM-DD+00:00:00")}"`,
  });
  tmpNode = await $find_elements({
    by: "id",
    value: "_easyui_textbox_input7",
    script: `$dom.value = "${dayjs(searchTime[1]).format("YYYY-MM-DD+23:59:59")}"`,
  });
  tmpNode = await $find_elements({
    by: "class name",
    value: "textbox-value",
  ...
  // 点击查询
  await $click({ by: "tag name", value: "input", array: 26 });
  await $timeout(10);
