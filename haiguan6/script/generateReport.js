const report = async () => {
  ...
  // 邮件详情
  await clickCheckMenu();
  await enterSearchTime(searchTime, 1, 3, 4, 0);
  ...
  await $to_default_content();
  await clickZhuanguanMenu();
  await enterSearchTime(searchTime, 2, 6, 7, 10);
  ...
  for (let mainNumber of mainNumberList) {
    ...
    // iframe.contentWindow.document.getElementsByClassName("mini-textbox-input")[0].value = mainNumber;
    await $click({
      by: "class name",
  ...
  let savePath = `C:/一局四中心邮递物品/dist/postdetails/${dayjs(searchTime[0]).format("YYYYMMDD")}/${dayjs(
    searchTime[0]
  ).format("HHmmss")}${dayjs(searchTime[1]).format("HHmmss")}.xlsx`;
  ...
  savePath = `C:/一局四中心邮递物品/dist/totalcode/${dayjs(searchTime[0]).format("YYYYMMDD")}/${dayjs(
    searchTime[0]
  ).format("HHmmss")}${dayjs(searchTime[1]).format("HHmmss")}.xlsx`;
  await $excel.read_excel({
    file: templatePath,
...
const enterSearchTime = async (searchTime, iframeIndex, dayStartIndex, dayEndIndex, confirmIndex) => {
  ...
  await $send_keys({
    ...
    value: "mini-buttonedit-input",
    array: dayStartIndex,
    txt: dayjs(searchTime[0]).format("YYYY-MM-DD HH:mm:ss"),
  });
  await $click({
    by: "class name",
    value: "mini-buttonedit-input",
  ...
  await $send_keys({
    ...
    value: "mini-buttonedit-input",
    array: dayEndIndex,
    txt: dayjs(searchTime[1]).format("YYYY-MM-DD HH:mm:ss"),
  });
  await $click({
    by: "class name",
