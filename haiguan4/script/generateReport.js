const report = async () => {
  ...
  for (const searchDate of searchDates) {
    ...
    let savePath = `D:/测试数据/dist/${dayjs(searchDate).format("YYYYMMDD")}/邮件详情/${dayjs(searchDate).format(
      "00时00分00秒"
    ...
    savePath = `D:/测试数据/dist/${dayjs(searchDate).format("YYYYMMDD")}/总包号/${dayjs(searchDate).format(
      "00时00分00秒"
    )}至${dayjs(searchDate).format("23时59分59秒")}.xlsx`;
    await $excel.read_excel({
      file: templatePath,
